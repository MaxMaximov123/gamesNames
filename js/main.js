...
async function convert_game_name(name1, name2, b1, b2, st) {
    data_for_db.push([name1, name2, st, b1, b2])
    // db.run(
...
async function add_data(response) {
    ...
    if (page % 100 === 0 || page >= response.data.pagination.lastPage){
        db.serialize(() => {
            db.run('BEGIN TRANSACTION');
            for (let d of data_for_db){
                db.run(
                    'INSERT INTO data(name1, name2, status, b1, b2) VALUES(?, ?, ?, ?, ?)',
                    d, (e, r)=>{});
            }
    ...
    if (page < response.data.pagination.lastPage) {
        console.log(page, '/', response.data.pagination.lastPage)
        page += 1;
